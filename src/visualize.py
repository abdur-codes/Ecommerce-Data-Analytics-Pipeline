...
def plot_outliers(df_original: pd.DataFrame, df_cleaned: pd.DataFrame, output_dir: str):
    ...
    exclude = ['order_year', 'order_month', 'order_day', 'order_dayofweek', 'order_quarter']
    plot_cols = [c for c in numeric_cols if c not in exclude][:4]

    if len(plot_cols) == 0:
        print("  No numeric columns for outlier plot")
        return
...
def plot_new_features(df: pd.DataFrame, output_dir: str):
    new_features = ['had_coupon', 'avg_item_price', 'is_weekend', 'order_month']
...
def plot_pipeline_summary(initial_shape, final_shape, missing_before, missing_after,
    ...
    plt.suptitle('E-Commerce Data Pipeline - Project Summary', fontsize=16, fontweight='bold')
    plt.tight_layout()
